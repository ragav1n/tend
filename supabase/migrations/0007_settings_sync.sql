-- ═══════════════════════════════════════════════════════════════════════════
-- 0007  let the settings row sync
--
-- user_settings has ridden along in sync_pull since 0003 and gone nowhere. The
-- client had no applier for it, and sync_writable_tables() never named it, so a
-- push mentioning it came back with a 42501 the client classifies as fatal and
-- retires. Every settings change a person made would have died in the deadletter.
--
-- Phase 2 turns that from a gap into a broken feature: the reminder pipeline
-- reads the timezone and the digest time out of this table, and the app is the
-- only thing that can set them. A digest at 07:00 UTC for somebody in Boston
-- arrives at 3am.
--
-- The row is a singleton keyed by user_id, with no id column and no tombstone, so
-- sync_push needs two things it did not have: an entityId it does not cast to
-- uuid, because the client calls this row 'me', and a key predicate on user_id.
-- Everything else, the field merge included, works on it unchanged.
-- ═══════════════════════════════════════════════════════════════════════════

/** user_settings joins the list. The push path above knows how to key it. */
...
as $$
  select array[
    'tasks', 'projects', 'tags', 'areas', 'task_series', 'task_tags', 'user_settings'
  ]::text[];
$$;
