-- ═══════════════════════════════════════════════════════════════════════════
-- 0005  one duplicate row must not poison the whole batch
--
-- Found by the two-client convergence test.
--
-- Two devices completing the same recurring task offline both generate
-- occurrence 5, and `unique (series_id, occurrence_seq)` is what stops both of
-- them landing. 0001 says the loser "gets a 23505 it resolves by accepting the
-- server's row", and lib/sync/errors.ts says the same thing. Neither was true:
-- the exception escaped sync_push, rolled the entire transaction back, and the
-- client classified 23505 as fatal, so every mutation in that batch went
-- straight to the deadletter. One duplicate occurrence stranded whatever
-- unrelated work happened to be queued behind it, and the loser's local row sat
-- on the device forever.
--
-- The insert now runs inside its own block. A unique violation rolls back that
-- one statement and comes back as `superseded`, which is a status rather than a
-- failure: the row the client tried to create already exists under another id,
-- so the client drops its local copy and takes the server's on the next pull.
--
-- The task_tags insert gets the same treatment for a foreign key violation. Its
-- one cause is a join row pointing at a tag whose own insert was superseded, and
-- the task carries the authoritative tag set on every pull, so dropping the
-- mutation loses nothing the next pull does not replace.
-- ═══════════════════════════════════════════════════════════════════════════
    ...
    if v_op = 'insert' then
      if v_table = 'task_tags' then
        ...
        exception when unique_violation then
          ...
          -- block scopes the rollback to this one statement, so the rest of the
          -- batch still commits.
          v_status := 'superseded';
        end;
      end if;
