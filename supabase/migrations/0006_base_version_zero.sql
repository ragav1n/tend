-- ═══════════════════════════════════════════════════════════════════════════
-- 0006  an edit made before the first pull is not a stale edit
--
-- Found by the two-client convergence test, which could not make a task recur:
-- the series landed, and the update that linked the task to it came back merged
-- with series_id dropped.
--
-- The client holds rowVersion 0 for a row it created and has never pulled back,
-- so every update it queues for that row carries baseVersion 0. The insert
-- stamps field_versions with the row's first real version, which is above 0 for
-- every column, so the merge check dropped every field of every edit made
-- before the first successful pull. Add a task offline, fix its title, sync: the
-- title went back to what it was and the server called it a merge. The same
-- thing silently unlinked recurrence, project moves and tag edits.
--
-- baseVersion 0 now skips the merge check. It means "I have never seen a server
-- version of this row", which only the row's author can truthfully say, since a
-- pulled row always arrives stamped. A device that has fallen behind sends the
-- version it last saw, which is above zero, and still loses the field it did not
-- know had moved.
-- ═══════════════════════════════════════════════════════════════════════════
